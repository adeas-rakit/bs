...
export default function WasteTypesManagement({ isFormOpen, setIsFormOpen }: WasteTypesManagementProps) {
  ...
  const fetchWasteTypes = async () => {
    ...
    try {
      ...
      const url = unitFilter === 'all' ? '/api/waste-types' : `/api/waste-types?unitId=${unitFilter}`;
        ...
        <div className="space-y-4">
            {filteredWasteTypes.length > 0 ? filteredWasteTypes.map(wt => (
                <InfoCard
                    ...
                    expandedInfo={
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-x-4 gap-y-2 text-sm">
                            <div className="font-semibold text-gray-500 flex items-center"><Building className="w-3 h-3 mr-2"/> Unit</div><div>{wt.unit.name}</div>
                        </div>
                    }
                    actionButtons={
                        <>
              ...
              <EmptyState
                ...
                description="Tidak ada jenis sampah yang cocok dengan filter Anda. Coba lagi atau tambahkan jenis sampah baru."
              />
            )}
        </div>
    </div>
